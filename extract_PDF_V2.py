...
def extract_text_pdf(storage_file, pdf_files):
    ...
    for pdf_file in pdf_files:
        ...
        progress_bar.print_progress_bar(compteur, taille, prefix = 'Extraction PDF : ' + str(compteur) +  '/' + str(taille), suffix = '')
        compteur = compteur + 1
...
def parser_text(storage_file,pdf_file,text): 
    ...
    permis = []
    for p in listPermis:
        if re.match('PERMIS ?(?:DE CONDUIRE|TYPE)? ?:? ?(?:AM|BSR|A|A1|A2|B|B1|B2|BE|BVA|C|C1|CE|C1E|D|D1|D2|DE|DE1)',p):
            per = re.sub('PERMIS ?(?:DE CONDUIRE|TYPE)? ?:? ?((?:AM|BSR|A|A1|A2|B|B1|B2|BE|BVA|C|C1|CE|C1E|D|D1|D2|DE|DE1))', '\g<1>',p)
            permis.append(per)
    #Sites/Réseaux
    ...
    if re.match('2020-12-03(?:-|_)CV(?:-|_)[A-Z]+(?:-|_)[A-Za-zéïàèîôû_\-]+.pdf',pdf_file):
        ...
        if sexe == 'NULL': 
            ...
            for line in fichier: 
                ...
                if soundex.get_soundex_code(line.lower().split(';')[0].lower()) == soundex.get_soundex_code(prenom.lower()):
                    normalized_levenshtein = NormalizedLevenshtein()
                    dict_similitude[normalized_levenshtein.similarity(line.lower().split(';')[0].lower(), prenom.lower())] = line.lower().split(';')[1]
    ...
    create_requete(storage_file, pdf_file,mail,tel,adresse,age,prenom,nom,sexe,permis,site_res,'NULL','NULL',tabFormation,listCompetence,listLangues,listCentreInteret)
...
def create_requete(storage_file,pdf_file,mail,tel,adresse,age,prenom,nom,sexe,permis,site_res,date_naiss,nationalite,tabFormation,listCompetence,listLangues,listCentreInteret):
    """
    ...
    ids = pd.DataFrame({'id_adr': [new_id_adr], 'id_can':[new_id_can], 'id_cv':[new_id_cv], 'id_site' : [int(id_site)]}) 
    ...
    if adresse != 'NULL':
        ...
        out_file.write('EXEC INSERT_ADRESSES('+ str(id_adr) + ',' + num_adr + ',' + localite_adr + ',' + nomRue_adr + ',' + cp_adr + ',' + ville_adr + ',' + pays_adr + ',' + continent_adr + ');\n')
    #Insertion Candidats
    out_file.write('EXEC INSERT_CANDIDATS(\''+ str(id_can) + '\',' + str(id_adr) + ',' + nom + ',' + prenom + ',' + sexe + ',' + age + ',' + date_naiss + ',' + mail + ',' + nationalite +  ',' + tel + ');\n')
    #Insertion Permis
    for p in permis:
        p = p.strip().upper()  
        date_obtention = 'NULL'
    ...
    for s in site_res: 
        ...
        #Insertion des id (pour éviter la redondance) 
        ids = pd.DataFrame({'id_adr': [new_id_adr], 'id_can':[new_id_can], 'id_cv':[new_id_cv], 'id_site' : [new_id_site]}) 
        ids.to_csv('./ids_tables.txt', index=False, header = True, mode = 'w')
    ...
    photo_cv = extract_photo(storage_file,pdf_file)
    nom_cv = unidecode.unidecode(pdf_file)

    out_file.write('EXEC INSERT_CV(\''+ str(id_cv) + '\',\'' + str(id_can)  +'\',\'' + nom_cv + '\','  + titre_cv + ',' + description_cv + ',' + posteRecherche_cv + ',' + typePoste_cv + ',' + dispo_cv + ',' + admis + ',' + date_transmission + ',' +  photo_cv +');\n')
    ...
    for formation in tabFormation:
        ...
        out_file.write('EXEC INSERT_SUIT_FORMATIONS(' + ecole + ',' + niveau + ',' + specialite + ',\'' + str(id_can) + '\',' + convert_date(formation[3]) + ',' + convert_date(formation[4]) + ');\n')
...
def extract_photo(storage_file, pdf_file):  
    """
...
if __name__ == '__main__':
    ...
    #Traitement des CV refusés
    print('Traitement des CV refusés')
    #Fichier de stockage des PDF que l'on veut traiter
    storage_file = './CV_REFUSE/'
    #Chargement des PDF
    list_pdf_files = loadURL(storage_file)
    #Extraction de l'informations des PDF et création des requetes SQL dans les fichiers SQL
    extract_text_pdf(storage_file,list_pdf_files)
